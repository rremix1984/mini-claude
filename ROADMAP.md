...
- **阶段 3.2: 基础命令** (100%)
  - 实现了 7 个基础命令
  - helpCommand - 显示帮助信息（支持详细用法和选项）
  - clearCommand - 清空当前对话
  - exitCommand - 退出程序（支持别名：quit, q）
  - configCommand - 配置管理（已存在）
  - historyCommand - 会话历史管理（已存在）
  - resumeCommand - 恢复会话（已存在）
  - logsCommand - 日志管理（已存在）

- **阶段 3.3: 开发命令** (100%)
  - 实现了 6 个开发命令
  - commitCommand - Git 提交（支持自动生成消息）
  - diffCommand - 查看 Git 变更（支持已暂存/未暂存）
  - testCommand - 运行测试（自动检测框架）
  - lintCommand - 代码检查（自动检测 linter）
  - buildCommand - 构建项目（自动检测构建系统）
  - runCommand - 运行脚本或命令

- **阶段 3.4: 高级命令** (100%)
  - 实现了 5 个高级命令
  - contextCommand - 显示当前上下文（系统、配置）
  - costCommand - 估算 API 成本（支持多种模型）
  - doctorCommand - 诊断环境问题（检查依赖和配置）
  - exportCommand - 导出会话（支持 JSON/Markdown/TXT 格式）
  - searchCommand - 搜索会话历史（支持限制和过滤）

### 🔄 下一步计划
- **阶段 4: 技能系统** (下一阶段)
  - 实现技能框架
  - 实现内置技能（代码审查、重构、测试生成等）
  - 实现技能管理命令
...
| 阶段 3.1: 命令框架 | ✅ 已完成 | 2026-04-01 | 2026-04-01 | CommandRegistry + 类型系统 |
| 阶段 3.2: 基础命令 | ✅ 已完成 | 2026-04-01 | 2026-04-01 | 7 个命令 (help, clear, exit, config, history, resume, logs) |
| 阶段 3.3: 开发命令 | ✅ 已完成 | 2026-04-01 | 2026-04-01 | 6 个命令 (commit, diff, test, lint, build, run) |
| 阶段 3.4: 高级命令 | ✅ 已完成 | 2026-04-01 | 2026-04-01 | 5 个命令 (context, cost, doctor, export, search) |
